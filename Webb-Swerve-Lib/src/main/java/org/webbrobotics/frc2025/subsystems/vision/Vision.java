...
public class Vision {
  ...
  public void periodic() {
    ...
    for (VisionIO camera : cameras) {
      ...
      Logger.recordOutput("Vision/" + cameraName + "/CameraTimeSeconds", result.getTimestampSeconds());
    }
  ...
  private void updateEstimationStdDevs(Optional<EstimatedRobotPose> estimatedPose, List<PhotonTrackedTarget> targets) {
    ...
    if (numTags == 0) {
      ...
      Matrix<N3, N1> baseStdDevs = (numTags > 1) 
  ...
  private void logSeenAprilTags() {
    ...
    int[] currentlyVisibleIds = currentlyVisibleTagIds.stream().mapToInt(Integer::intValue).sorted().toArray();
