...
public class AutoCommands {
  ...
  public static Command resetPoseCommand(HolonomicTrajectory trajectory, boolean mirror) {
    return Commands.runOnce(() -> resetPose(trajectory, mirror));
  }
   ...
   * @return A command that resets the robot pose
   */
  public static Command resetPoseCommand(Pose2d pose, boolean applyAllianceFlip) {
    return Commands.runOnce(
        () ->
            RobotState.getInstance()
                .resetPose(applyAllianceFlip ? AllianceFlipUtil.apply(pose) : pose));
  }
